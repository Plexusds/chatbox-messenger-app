...
// Creating the console debug for loggin purposes
...
function receivedMessage(event) {
  ...
  console.debug("Received message for user %d and page %d at %d with message:",
 ...
 *
 */
function sendToKafka(senderID, message) {
  //Message to send
  var producerRequest =
    {
      topic: 'messenger-message-inbox',
      messages: JSON.stringify(message),
      key: senderID,
      timestamp: Date.now()
    };
  producer.send([producerRequest], function (err, data) {
    if (err == null)
      console.debug(`Sending message to kafka.\nMessage:\n${JSON.stringify(producerRequest)}`);
    else {
      console.error(`Error sending message to kafka. Error: ${err}`);
      sendTextMessage(senderID, "Sorry, We couldn't handle your petition. Try again later.");
    }
  });
}
...
if (DEBUG) {
   ...
   */
    ...
    KAFKA VERSION 0.10.2.0 (DOCKER IMAGE OF confluentinc/cp-enterprise-kafka:3.2.1)
    ...
    THIS ONLY HAPPENS WHEN KAFKA AND THIS APP IS EXECUTED AT THE SAME TIME.
...
producer
  .on('ready', function () {
    console.log('kafka producer ready to send messages');
  })
